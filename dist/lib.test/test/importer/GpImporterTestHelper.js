...
export class GpImporterTestHelper {
    static prepareImporterWithFile(name, settings = null) {
        return __awaiter(this, void 0, void 0, function* () {
            let path = 'test-data/';
            const buffer = yield TestPlatform.loadFile(path + name);
            return GpImporterTestHelper.prepareImporterWithBytes(buffer, settings);
        });
    }
    static prepareImporterWithBytes(buffer, settings = null) {
        let readerBase = new Gp3To5Importer();
        readerBase.init(ByteBuffer.fromBuffer(buffer), settings !== null && settings !== void 0 ? settings : new Settings());
        return readerBase;
    }
    static checkNotes(score) {
        // Whole Notes
        let beat = 0;
        let durationsInFile = [
            Duration.Whole,
    ...
    static checkTimeSignatures(score) {
    ...
    static checkAccentuations(score, includeHeavy) {
        expect(score.tracks[0].staves[0].bars[0].voices[0].beats[0].notes[0].isGhost).toBe(true);
        expect(score.tracks[0].staves[0].bars[0].voices[0].beats[1].notes[0].accentuated).toEqual(AccentuationType.Normal);
        if (includeHeavy) {
            expect(score.tracks[0].staves[0].bars[0].voices[0].beats[2].notes[0].accentuated).toEqual(AccentuationType.Heavy);
        }
        expect(score.tracks[0].staves[0].bars[0].voices[0].beats[3].notes[0].isLetRing).toBe(true);
    }
    static checkHarmonics(score) {
    ...
    static checkSlides(score) {
        expect(score.tracks[0].staves[0].bars[0].voices[0].beats[0].getNoteOnString(5).slideOutType).toEqual(SlideOutType.Legato);
        expect(score.tracks[0].staves[0].bars[0].voices[0].beats[2].getNoteOnString(2).slideOutType).toEqual(SlideOutType.Shift);
        expect(score.tracks[0].staves[0].bars[1].voices[0].beats[0].getNoteOnString(5).slideInType).toEqual(SlideInType.IntoFromBelow);
        expect(score.tracks[0].staves[0].bars[1].voices[0].beats[1].getNoteOnString(5).slideInType).toEqual(SlideInType.IntoFromAbove);
        expect(score.tracks[0].staves[0].bars[1].voices[0].beats[2].getNoteOnString(5).slideOutType).toEqual(SlideOutType.OutDown);
        expect(score.tracks[0].staves[0].bars[1].voices[0].beats[3].getNoteOnString(5).slideOutType).toEqual(SlideOutType.OutUp);
    }
    static checkStrings(score) {
    ...
    static checkVibrato(score, checkNotes) {
        if (checkNotes) {
    ...
    static checkOtherEffects(score, skipInstrumentCheck = false) {
    ...
    static checkFingering(score) {
        ...
        expect(score.tracks[0].staves[0].bars[0].voices[0].beats[6].notes[0].rightHandFinger).toEqual(Fingers.IndexFinger);
        expect(score.tracks[0].staves[0].bars[0].voices[0].beats[7].notes[0].rightHandFinger).toEqual(Fingers.MiddleFinger);
        expect(score.tracks[0].staves[0].bars[0].voices[0].beats[8].notes[0].rightHandFinger).toEqual(Fingers.AnnularFinger);
        expect(score.tracks[0].staves[0].bars[0].voices[0].beats[9].notes[0].rightHandFinger).toEqual(Fingers.LittleFinger);
    }
    static checkStroke(score) {
    ...
    static checkEffects(score) {
        // just check if reading works
        expect(true).toBe(true);
    }
    static checkKeySignatures(score) {
        // major - flats
    ...
    static createChord(name, firstFret, strings, barreFrets) {
    ...
    static checkChords(score) {
        let track = score.tracks[0];
        let staff = track.staves[0];
        expect(staff.chords.size).toEqual(8);
        GpImporterTestHelper.checkChord(GpImporterTestHelper.createChord('C', 1, [0, 1, 0, 2, 3, -1]), track.staves[0].bars[0].voices[0].beats[0].chord);
        GpImporterTestHelper.checkChord(GpImporterTestHelper.createChord('Cm', 1, [-1, -1, 0, 1, 3, -1]), track.staves[0].bars[0].voices[0].beats[1].chord);
        GpImporterTestHelper.checkChord(GpImporterTestHelper.createChord('C', 1, [3, 5, 5, 5, 3, -1], [3]), track.staves[0].bars[0].voices[0].beats[2].chord);
        GpImporterTestHelper.checkChord(GpImporterTestHelper.createChord('Cm', 1, [3, 4, 5, 5, 3, -1], [3]), track.staves[0].bars[0].voices[0].beats[3].chord);
        GpImporterTestHelper.checkChord(GpImporterTestHelper.createChord('D', 1, [2, 3, 2, 0, -1, -1], [2]), track.staves[0].bars[1].voices[0].beats[0].chord);
        GpImporterTestHelper.checkChord(GpImporterTestHelper.createChord('Dm', 1, [1, 3, 2, 0, -1, -1]), track.staves[0].bars[1].voices[0].beats[1].chord);
        GpImporterTestHelper.checkChord(GpImporterTestHelper.createChord('D', 5, [5, 7, 7, 7, 5, -1], [5]), track.staves[0].bars[1].voices[0].beats[2].chord);
        GpImporterTestHelper.checkChord(GpImporterTestHelper.createChord('Dm', 5, [5, 6, 7, 7, 5, -1], [5]), track.staves[0].bars[1].voices[0].beats[3].chord);
    }
    static checkChord(expected, actual) {
...
//# sourceMappingURL=GpImporterTestHelper.js.map