...
export class MusicXmlImporterTestHelper {
    static prepareImporterWithBytes(buffer) {
        let readerBase = new MusicXmlImporter();
        readerBase.init(ByteBuffer.fromBuffer(buffer), new Settings());
        return readerBase;
    }
    static testReferenceFile(file, renderLayout = LayoutMode.Page, renderAllTracks = false) {
        return __awaiter(this, void 0, void 0, function* () {
            const fileData = yield TestPlatform.loadFile(file);
            try {
                let importer = MusicXmlImporterTestHelper.prepareImporterWithBytes(fileData);
    ...
    expectScoreEqual(expected, actual) {
        ...
        for (let i = 0; i < expected.masterBars.length; i++) {
    ...
    expectTrackEqual(expected, actual) {
        ...
        for (let i = 0; i < expected.staves.length; i++) {
            this.expectStaffEqual(expected.staves[i], actual.staves[i]);
        }
    }
    expectStaffEqual(expected, actual) {
        ...
        for (let i = 0; i < expected.bars.length; i++) {
            this.expectBarEqual(expected.bars[i], actual.bars[i]);
        }
    }
    expectBarEqual(expected, actual) {
        expect(actual.index).toEqual(expected.index, 'Mismatch on Index');
        ...
        for (let i = 0; i < Math.min(expected.voices.length, actual.voices.length); i++) {
            this.expectVoiceEqual(expected.voices[i], actual.voices[i]);
        }
    }
    expectVoiceEqual(expected, actual) {
        expect(actual.index).toEqual(expected.index, 'Mismatch on Index');
    ...
    expectBeatEqual(expected, actual) {
        expect(actual.index).toEqual(expected.index, 'Mismatch on Index');
        ...
        for (let i = 0; i < expected.notes.length; i++) {
            this.expectNoteEqual(expected.notes[i], actual.notes[i]);
        }
    }
    expectNoteEqual(expected, actual) {
        expect(actual.index).toEqual(expected.index, 'Mismatch on Index');
    ...
    expectChordEqual(expected, actual) {
        expect(!expected).toEqual(!actual);
        if (expected) {
            // expect(actual.name).toEqual(expected.name, 'Mismatch on Name');
        }
    }
    expectBendPointsEqual(expected, actual) {
    ...
    expectPlaybackInformationEqual(expected, actual) {
    ...
    expectMasterBarEqual(expected, actual) {
        ...
        expect(actual.timeSignatureNumerator).toEqual(expected.timeSignatureNumerator, 'Mismatch on TimeSignatureNumerator');
        expect(actual.timeSignatureDenominator).toEqual(expected.timeSignatureDenominator, 'Mismatch on TimeSignatureDenominator');
    ...
    expectSectionEqual(expected, actual) {
...
//# sourceMappingURL=MusicXmlImporterTestHelper.js.map