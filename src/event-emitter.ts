...
function setupListeners(renderer: IScoreRenderer) {
    $(window).on('alphaTab.beatMouseDown', (event, beat) => {
        ...
        const bounds = renderer.boundsLookup.getNoteBounds(note);
        if (bounds) {
        emitEvent({
...
export default class EventEmitter{
    constructor (renderer: IScoreRenderer, callback: (arg: EditorUIEvent) => any) {
